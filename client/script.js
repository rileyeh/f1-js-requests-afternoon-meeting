...
function createFood(e) {
    ...
    axios.post('http://localhost:3000/food', body) 
        .then(function(res) {
            // console.log(res)
            let lastValue = res.data.pop()
            let foodText = document.createElement('p')
            foodText.textContent = lastValue
            document.body.appendChild(foodText)
        })
...
// anonymous arrow function (sometimes you'll see these without the parameter parentheses if there is only 1 argument)
// () => {
